...
export default function Message() {
  ...
  async function getProfile() {
    const { data } = await axios.get(`https://lit-caverns-07639.herokuapp.com/profile/${params.id}`);
  ...
  async function sendData(e) {
    ...
    if (result.error) {
      ...
      setIsLoading(true);
      setError([]);
      const { data } = await axios.post(`https://lit-caverns-07639.herokuapp.com/message/${params.id}`, messageBody);
      if (data.message === "Done") {
        setIsLoading(false);
        setIsDone(true);
      } else {
                ...
                {isDone ? (
                  ...
                  <>
                    <div className="mt-4 d-flex justify-content-center align-items-center flex-column text-center">
                      <img className="rounded " src={`https://lit-caverns-07639.herokuapp.com/${profile.profilePic}`} alt="profilepicture" />
