...
export default function Message() {
  ...
  async function getProfile() {
    const { data } = await axios.get(`https://saraha-api.fly.dev/profile/${params.id}`);
  ...
  async function sendData(e) {
    ...
    if (result.error) {
      ...
      setIsLoading(true);
      setError([]);
      const { data } = await axios.post(`https://saraha-api.fly.dev/message/${params.id}`, messageBody);
      if (data.message === "Done") {
        setIsLoading(false);
        setIsDone(true);
      } else {
                ...
                {isDone ? (
                  ...
                  <>
                    <div className="mt-4 d-flex justify-content-center align-items-center flex-column text-center">
                      <img className="rounded " src={`https://saraha-api.fly.dev/${profile.profilePic}`} alt="profilepicture" />
