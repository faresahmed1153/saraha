...
export default function Home() {
  ...
  async function getData() {
    const { data } = await axios.get("https://saraha.up.railway.app/messages", {
  ...
  async function deleteMessage(messageId) {
    const { data } = await axios.delete(`https://saraha.up.railway.app/message/${messageId}`, {
      headers: { authorization: `Bearer ${token}` },
    });
    console.log(data);
      ...
      <div className="container mt-3 ">
        <div className="row justify-content-center align-items-center flex-column  ">
          <div className="text-center mt-5">
            ...
            <h5 className="word-wrap">{`https://saraha.netlify.app/messageMe/${id}`}</h5>
          </div>
